<button type="{{ $type }}" class="relative {{ $flag == 1 ? 'bg-gray-400' : 'border-'.$class }} px-4 py-[6px] rounded-full text-white group overflow-hidden border-2 {{ $flag == 1 ? 'bg-gray-400' : 'bg-' . $class }}" id={{$id}} {{ $flag == 1 ? 'disabled' : '' }}>
